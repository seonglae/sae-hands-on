...
class NNetSAE(BaseAutoencoder):
    def __init__(self, cfg):
        super().__init__(cfg)
        self.intermediate_size = cfg["intermediate_size"]
        self.W_int = nn.Parameter(
            torch.nn.init.kaiming_uniform_(
                torch.empty(self.cfg["act_size"], self.intermediate_size)
            )
        )
        self.W_enc = nn.Parameter(
            torch.nn.init.kaiming_uniform_(
                torch.empty(self.intermediate_size, self.cfg["dict_size"] - self.intermediate_size)
            )
        )
        self.to(cfg["dtype"]).to(cfg["device"])
    ...
    def forward(self, x):
        ...
        x_cent = x - self.b_dec
        acts_int = F.relu(x_cent @ self.W_int)
        acts_int_topk = torch.topk(acts_int.flatten(), int(self.cfg["intermediate_top_k"]) * x.shape[0], dim=-1)
        acts_int_topk = (
            torch.zeros_like(acts_int.flatten())
            .scatter(-1, acts_int_topk.indices, acts_int_topk.values)
            .reshape(acts_int.shape)
        )
        if self.cfg['residual_connection'] and self.intermediate_size == self.cfg['act_size']:
            acts = F.relu((acts_int_topk + x_cent) @ self.W_enc)
        else:
            acts = F.relu(acts_int_topk @ self.W_enc)
        acts_topk = torch.topk(acts.flatten(), int(self.cfg["top_k"]) * x.shape[0], dim=-1)
        acts_topk = (
            torch.zeros_like(acts.flatten())
            .scatter(-1, acts_topk.indices, acts_topk.values)
            .reshape(acts.shape)
        )
        concat_acts = torch.cat((acts_int_topk, acts_topk), dim=-1)
        x_reconstruct = concat_acts @ self.W_dec + self.b_dec

        self.update_inactive_features(concat_acts)
        output = self.get_loss_dict(x, x_reconstruct, acts, acts_topk, acts_int, acts_int_topk, x_mean, x_std)
        return output

    def get_loss_dict(self, x, x_reconstruct, acts, acts_topk, acts_int, acts_int_topk, x_mean, x_std):
        l2_loss = (x_reconstruct.float() - x.float()).pow(2).mean()
        l1_norm = acts_topk.float().abs().sum(-1).mean()
        l1_norm_int = acts_int_topk.float().abs().sum(-1).mean()
        l1_loss = self.cfg["l1_coeff"] * l1_norm
        l1_loss_int = self.cfg["l1_coeff"] * l1_norm_int * (self.intermediate_size / (self.cfg["dict_size"] - self.intermediate_size))
        l0_norm = (acts_topk > 0).float().sum(-1).mean()
        l0_norm_int = (acts_int_topk > 0).float().sum(-1).mean()
        aux_loss = self.get_auxiliary_loss(x, x_reconstruct, torch.cat((acts, acts_int), dim=-1))
        loss = l2_loss + l1_loss + aux_loss + l1_loss_int
        num_dead_features = (
        ...
        output = {
            ...
            "l0_norm": l0_norm,
            "l1_norm": l1_norm,
            "l0_norm_int": l0_norm_int,
            "l1_norm_int": l1_norm_int,
            "aux_loss": aux_loss,
        }
        return output
...
class SymSAE(TopKSAE):
    def __init__(self, cfg):
        super().__init__(cfg)
        if "W_dec" in self._parameters:
            del self._parameters["W_dec"]
        if "W_dec" in self.__dict__:
            del self.__dict__["W_dec"]
